    ...
    <div className="min-h-screen bg-[#D5E8EB]">
      ...
      <div className="relative w-full flex-1 font-['Alata',sans-serif]" style={{ minHeight: "calc(100vh - 84px)", isolation: "isolate" }}>
        ...
        {(selectedState || showFavoritesPanel) && (() => {
          // Favoritos tiene prioridad visual cuando está activo, incluso si hay estado seleccionado
          const isFavMode = showFavoritesPanel;
          const accentColor = isFavMode ? "#FFE74C" : "#3BCEAC";

          return (
            <div
              className="absolute top-0 right-0 bottom-0 z-[999] w-72 overflow-y-auto flex flex-col"
              style={{
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    {isFavMode ? (
                      ...
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#3BCEAC" strokeWidth="2">
                    ...
                    <h3 className="text-[#6E2594] text-sm font-bold tracking-wide">
                      {isFavMode
                        ? "Mis Favoritos"
                        : MEXICO_STATES.find((s) => s.id === selectedState)?.name || "Lugares Turísticos"}
                    </h3>
                  </div>
                  {/* Botón cerrar: cierra el panel activo */}
                  <button
                    onClick={() => {
                      if (isFavMode) {
                        setShowFavoritesPanel(false);
